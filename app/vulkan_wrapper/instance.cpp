...
namespace FF::Wrapper {
	...
	Instance::Instance(bool enableValidationLayer){
		...
		if (_enableValidationLayer && !checkValidationLayerSupport()) {
			throw std::runtime_error("Error: validaton layer is not supported");
		}
		...
		appInfo.pApplicationName = "vulkan_lesson";
		...
		if (vkCreateInstance(&instCreateInfo, nullptr, &_instance) != VK_SUCCESS) {
	...
	Instance::~Instance() {
		if (_enableValidationLayer) {
			DestroyDebugUtilsMessengerEXT(_instance, _debugger, nullptr);
		}
	...
	void Instance::setupDebugger() {
		...
		if (CreateDebugUtilsMessengerEXT(_instance, &createInfo, nullptr, &_debugger) != VK_SUCCESS) {
